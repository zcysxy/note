...
Consider a [[Statistical Model]] parameterized by a parameter $\theta$ and an observation of samples $X \sim P\_{\theta}$. We have, the **probability** of observing $X$ under parameter $\theta$ is $\Pr(X \mid \theta)$.
...
For iid samples ${ x\_i }$, the likelihood function is of the form $L(\theta) = \prod f(x\_{i}\mid \theta)$. Thus, it is usually more convenient to deal with the log of the likelihood function, turning the product into a sum:

$$
\ell(\theta)\coloneqq \log L(\theta) = \log \prod\_{i} f(x\_{i}\given \theta) = \sum\_{i} \log f(x\_{i}\given \theta).
$$
