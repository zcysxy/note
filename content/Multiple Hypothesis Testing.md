...
That is, unlike Bonferroni, BH does not reject nulls under a function graph. Instead, it first determines $i_{0}$, and then reject nulls before $i_{0}$ on the line of sorted p-values. See [[#^fig-bh]] for an illustration: even before $i_{0}$ there are some p-values above the line $\frac{i}{n}\alpha$, they get rejected because $i_{0}$ is the last index below the line.
