...
created: 2025-06-06T02:05:38
modified: 2025-06-06T03:50:21
...
- An estimator $\hat{\theta}_m$ is said to be ==asymptotically unbiased== if $\lim_{m \to \infty} \operatorname{bias}(\hat{\theta}\_m)=0$.
  - Asymptotic unbiasedness is NOT equivalent to [[#Consistency vs Asymptotic Unbiasedness|consistency]].
...
$$\hat{\sigma}^2\_m = \frac{1}{m}\sum\_{i=1}^m(x^{(i)} - \hat{\mu}\_m)^2 $$
...
$$
\tilde{\sigma}^2\_m = \frac{1}{m-1}\sum\_{i=1}^m(x^{(i)} - \hat{\mu}\_m)^2 = \frac{m}{m-1}\hat{\sigma}^2\_m
$$

From the calculation above, we know that unbiased sample variance estimator $\tilde{\sigma}^2\_m$ is **unbiased**.
...
Using $\hat{\mu}$ to denote the mean of the data, from the calculation in [[#Example Variance Estimator]], we have
...
For example, [[Mean Squared Error]] is the quadratic risk using a quadratic loss function $L(\hat{\theta},\theta) = (\hat{\theta} - \theta)^2$; and it encompasses both [[#Bias]] and [[#Standard Error]]: $\mathrm{MSE} = \mathrm{Bias}^{2} + \mathrm{SE}^{2}$.
...
The above can be proved using [[Chebyshev Inequality]] or the [[Convergence of Random Variables#Relation between Convergence Modes]] ($L\_2$ convergence implies convergence in probability).
...
For an asymptotic normal estimator, its _statistical difficulty_[^1] is governed by its asymptotic variance $\Sigma$.
