...
[[R Type]]s are types for the smallest objects in [[R]]. Furthermore, these objects can form more complex **data structures**.
...
Use square brackets `[]` and index (_**starting from 1**_) to fetch the elements from a vector. Use a colon `:` to specify a range.
