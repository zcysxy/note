...
- Remark: Convergence in probability has a similar interpretation as almost sure convergence, and it also generally requires  to be dependent. However, it's weaker than almost sure convergence as it's not _uniform_: different $(\epsilon,\delta)$ pairs such that $\P(|X_{n}-X| > \epsilon) \le \delta$ require different $n$.
...
- Alternative interpretation: Weak convergence inspects the convergence of the associated CDF of a random variable sequence, and thus it's _weaker_ and generally requires no dependency between $X_{n}$. It's equivalent to the convergence of the [[Characteristic Function]].
