...
`fplot` is the function to plot expressions or functions.
...
- `fplot(funx,funy,<tinterval>)` plots the curve defined by `x = funx(t)` and `y = funy(t)` over the specified interval for _t_
  - default interval $[-5, 5]$
...
- Plot Piecewise Functions and Specify Line Properties
