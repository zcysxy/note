...
Recall (see [[#Bayes Estimator]]) that given a normal prior $Q_{\tau}=\mathcal{N}(0,\tau^{2}I_{d})$, the posterior is $\mathcal{N}\bigl( \hat{\theta}_{\mathrm{post}}, \bigl( \tfrac{\xx}{\sigma^{2}} + \tfrac{I}{\tau^{2}} \bigr)^{-1} \bigr)$, and the Bayes risk w.r.t $Q_{\tau}$ is
...
Therefore, we are left to show that for any general estimator $\hat{\theta}(Y)$ corresponding to the original Gaussian linear model, there exists a induced estimator $A(U^TY)$ such that $R_{d}(A,\theta) = R(\hat{\theta},\theta)$. We claim this is true with the following _randomized_ induced estimator:
