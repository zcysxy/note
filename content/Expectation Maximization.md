...
published: 2026-01-07T01:10:18.071Z
tags:
  - pub-stat
type: note
sup:
...
Continuing the [[#^ex2|mixture of Gaussians]] example, we see that given the hidden observation $\{ y_i \}_{i=1}^{n}$, we can easily calculate the log-likelihood maximizer:
...
For the E-step, since $[x_{i}^{o}; x_{i}^{m}] \sim \mathcal{N}(\mu,\Sigma)$, by [[Normal Marginal Distribution from Normal Joint Distribution]], we get the conditional parameter of [[#^q]]:
