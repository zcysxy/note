...
published: 2026-01-07T01:10:17.999Z
tags:
  - pub-stat
type: note
sup:
...
Sometimes $\Theta$ may not possess a [[Sigma Field]] that allows us to define a uniform distribution, for example, when $\Theta$ is unbounded or countably infinite. The principle still holds that we want the prior to assign equal _measure_ to all elements in $\Theta$.
...
- Frequentists believe in empirical data and use probability distributions to _model_ the observed data.
- Bayesians believe there is an underlying distribution, and use samples to _infer_ that distribution.
...
![The "density" plot of the answer from frequentists and Bayesians.](bayes.excalidraw.svg)
