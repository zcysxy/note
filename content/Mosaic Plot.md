...
published: 2026-05-11T15:41:06.514Z
tags:
  - pub-edav
type: note
dg-publish: true
...
- Mosaic plots are like stacked [[Bar Chart]]s, except that the width and height of the bars are proportional to the amount of data.
...
- Mosaic plots with the same bin width are relative frequency stacked [[Bar Chart]]s
...
The _steeper_ the _stairs_, the stronger the relationship.
...
Just like [[Scatterplot Matrix]], we can make mosaic plot matrix, with each element being a spine plot with two variables. Then for $n$ variables, we need an $n\times n$ matrix. A diagonal elements is a vertical proportion cut of one variable.
...
- `mosaic()` in package `vcd`
  ...
  - Here [[R Type - Formula]] can be read as "on", especially for dependent variables ^hv0kpp
