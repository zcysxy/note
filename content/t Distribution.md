...
$$
T\_n = \frac{Z}{\sqrt{ \chi \_{n}^{2} /n }},
$$
where $Z \sim \mathcal{N}(0,1)$. $T\_n$ is said to have a **t-distribution** with $n$ degrees of freedom.

By the [[Law of Large Numbers]], for large $n$, $T\_n \approx Z \sim \mathcal{N}(0,1)$ (but always with larger [[Variance]]).
...
Suppose ${ X\_{i}\sim \mathcal{N}(\mu,\sigma^{2}) }$, then
$$
...
Let ${ X\_{i}\sim \mathcal{N}(\mu\_{1}, \sigma^{2}) }_{i=1}^{n}$ and ${X\_j \sim \mathcal{N}(\mu_{2},\sigma^{2}) }_{j=1}^{m}$. Then
$$
\frac{(\overline{x}-\overline{y}) - (\mu_{1}-\mu\_{2})}{s\_{p} \sqrt{ \frac{1}{m}+\frac{1}{n} }} \sim t _{m+n-2},
$$
where
$$
...
Let ${ X\_{i}\sim \mathcal{N}(\mu\_{1}, \sigma\_{1}^{2}) }_{i=1}^{n}$ and ${X\_j \sim \mathcal{N}(\mu_{2},\sigma\_{2}^{2}) }_{j=1}^{m}$. Then **approximately**, we have
$$
\frac{(\overline{x} - \overline{y}) - (\mu_{1} - \mu\_{2})}{\sqrt{ \frac{s^{2}_{1}}{n} + \frac{s_{2}^{2}}{m} }} \sim t _{N},
$$
where
$$
N = \frac{\left( \frac{s_{1}^{2}}{n} + \frac{s\_{2}^{2}}{m} \right) ^{2}}{\frac{s\_{1}^{4}}{n^{2}(n-1)} + \frac{s\_{2}^{4}}{m^{2}(m-1)}} \gtrsim \min { n,m }.
$$
