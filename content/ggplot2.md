...
published: 2026-05-09T18:15:16.154-04:00
tags:
  - pub-edav
...
state: done
source: |
  "https://r4ds.had.co.nz/data-visualisation.html", [Chapter 7 Learning ggplot2 | edav.info, 2nd edition](https://edav.info/learning-ggplot2.html), [Create a new layer — layer • ggplot2](https://ggplot2.tidyverse.org/reference/layer.html)
---
...
A geom is the geometric object to represent data, and is the most crucial part of a layer, because different geoms make _totally different_ plots.
...
Not all graphs plot the same information about the data. Scatter plots plot the raw value, while [[Histogram]]s plots the frequencies. The algorithm used to calculate the statistics to plot is called a `stat`, short for statistical transformation.
...
A scale controls how data is mapped to aesthetic attributes, so one scale for one [[#Aesthetic Mapping]].
...
- Use `facet_grid()` with [[R Type - Formula]]--`x ~ y`; then `x` will be mapped to rows and `y` to columns ^b6alii
