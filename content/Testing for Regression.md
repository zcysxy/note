...
[[Regression]] studies the effect of the explanatory variable $X$ on the response variable $Y$. After solving the regression task, we want to know if specific features of $X$ have _significant_ effects on $Y$. We use [[Hypothesis Testing]] for this purpose.

For example, in a linear regression model:
$$
Y = f\_{\theta}(X) + \epsilon = X\theta +\epsilon ,
$$
we want to test
$$
H\_{0}: \theta _{j} = 0 \quad \text{vs} \quad H_{1}: \theta \_{j} \neq 0.
$$
^ex
...
We focus the [[#^ex|above example]] with a [[Gaussian Linear Model]], i.e., $\epsilon \sim \mathcal{N}(0,\sigma^{2}I)$.
...
$$
T\_{n}^{(j)} \coloneqq \frac{\frac{\hat{\theta}^{\mathrm{LS}}_{j} - \theta _{j}}{\sqrt{ \sigma^{2}(X^TX)^{-1}_{jj} }}}{\sqrt{ \frac{\hat{\sigma}^{2}}{\sigma^{2}} }} = \frac{\hat{\theta}^{\mathrm{LS}}_{j}-0}{\sqrt{ \hat{\sigma}^{2} (X^TX)^{-1}_{jj} }} \sim t_{n-d},
$$
where $A\_{j j}$ is the $j$-th diagonal element of the matrix $A$.
