...
A Bayes optimal _procedure_ $A^{\*}$ minimizes the [[Risk|Bayes risk]]:
$$
...
$$
R(A) = \mathbb{E}_{\theta \sim  Q}\mathbb{E}_{X \sim P\_{\theta}}\[L(A(X),\theta)].
$$
...
$$
R(A) = \pi\_{0}P\_{\theta\_{0}}(A(X)=1) \cdot c\_{\mathrm{FP}} + \pi\_{1}P\_{\theta\_{1}}(A(X)=0) \cdot c\_{\mathrm{FN}},
$$
where $\pi _{0},\pi_{1}$ are the priors of the two hypotheses; $c\_{\mathrm{FP}},c\_{\mathrm{FN}}$ are the costs of false positive and false negative.
...
A Bayes optimal procedure _greedily_ minimizes the "individual loss":
$$
A^{\*}(x) = \arg\inf\_{\hat{y}} \mathbb{E}\_{P \sim Q, Y \sim P|X}\[L(\hat{y},Y)\given X=x], \quad \forall x\in \mathcal{X}.
$$
