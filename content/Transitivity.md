...
published: 2026-04-30T15:37:14.385-04:00
tags:
  - "#pub-network"
state: done
sup:
...
$$
C\_{i} = \frac{\text{number of triangles connected to }i}{\text{number of triples centered on }i}
\= \frac{|{ (j,k)\in E : j,i\in \mathcal{N}_{i}}|}{\deg(i)(\deg(i)-1)/2}.
$$
We use the convention that $C\_i = 0$ if $\deg(i) < 2$.
Then, we can define the average clustering coefficient of the graph as $\overline{C} = \frac{1}{n}\sum_{i}C\_{i}$.
...
$$
C = \frac{\sum\_{i,j,k} \mathbb{1}{ (i,j),(j,k),(k,i) \in E}}{\sum\_{i,j,k} \mathbb{1}{ (i,j),(j,k)\in E }}.
$$
...
The overall clustering can be thought of a weighted averaging of clustering across nodes with weights proportional to $d\_i\choose 2$, while average clustering weights all nodes equally.
